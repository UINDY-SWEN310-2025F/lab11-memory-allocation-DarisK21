...
void bestFit(int nSeg, int nProc, int segSize[], int segUsed[], int segStart[], AllocationInfo alloc[]) {
    ...
    for(int p = 0; p < nProc; p++) {
        int bestIdx   = -1;
        int bestWaste = __INT_MAX__;   

        for(int s = 0; s < nSeg; s++) {
            if(!occupied[s] && segSize[s] >= alloc[p].size) {
                int waste = segSize[s] - alloc[p].size;
                if(waste < bestWaste) {
                    bestWaste = waste;
                    bestIdx   = s;
                }
        ...
        if(bestIdx != -1) {
            alloc[p].partition = bestIdx;
            alloc[p].start     = segStart[bestIdx];
            alloc[p].end       = segStart[bestIdx] + alloc[p].size;
            occupied[bestIdx]  = 1;
            segUsed[bestIdx]   = alloc[p].size;
        }
    }
